...
unsigned int nStakeMaxAge = 60 * 60 * 8;	// stake age of full weight: 8 Hours
unsigned int nStakeTargetSpacing = 30;			// 30 sec block spacing

int64 nChainStartTime = 1396841542;
...
const string strMessageMagic = "CapitalistCoin Signed Message:\n";
...
{
    return (nHeight > MAX_POW_BLOCK ? 1 : 50000) * COIN;
}
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake) 
{
	...
	/*
	printf(">> Height = %d, fProofOfStake = %d, nInterval = %"PRI64d", nStakeTargetSpacing = %"PRI64d", nActualSpacing = %"PRI64d"\n", 
		pindexPrev->nHeight, fProofOfStake, nInterval, nStakeTargetSpacing, nActualSpacing);  
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in CapitalistCoin
    bool fStrictPayToScriptHash = true; // Always active in CapitalistCoin
...
bool CBlock::AcceptBlock()
{
    ...
    // Ignore in testnet for testing. 
    // Proof of Stake only after premined blocks are finished.
    if (!fTestNet && (IsProofOfStake() && nHeight < MAX_POW_BLOCK))
        return DoS(100, error("AcceptBlock() : Proof-of-stake not allowed."));

    // Ignore in testnet for testing. 
    // Disallow proof of work after initial blocks.
    if (!fTestNet && (IsProofOfWork() && nHeight >= MAX_POW_BLOCK))
        return DoS(100, error("AcceptBlock() : Proof-of-work not allowed."));
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "CapitalistCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        nStakeMaxAge = 10 * 60; // test net min age is 10 min
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "April 7th 2014 Capitalism never dies";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1396841542;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 162291;

        if (fTestNet)
        {
            block.nTime    = 1396726124;
        ...
        assert(block.hashMerkleRoot == uint256("0xde93df7071eff80f36ae2f46c3c77d041e110aa67610f57e5ac0e903def7e2cc"));
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    if (fProofOfStake)  // attempt to find a coinstake
    {
        ...
        {
			// printf(">>> OK1\n");
            if (pwallet->CreateCoinStake(*pwallet, pblock->nBits, nSearchTime-nLastCoinStakeSearchTime, txCoinStake, nFees))
            {
